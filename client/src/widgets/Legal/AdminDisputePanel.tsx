...
export default function AdminDisputePanel() {
    ...
    const resolveMutation = useMutation({
        mutationFn: ({ id, resolution, status }: { id: string; resolution: string; status: 'resolved' | 'dismissed' }) =>
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            ...
            case 'under_review': return <Badge className="bg-yellow-500">Reviewing</Badge>;
    ...
    const openDisputes = (disputes || []).filter((d: Dispute) => d.status === 'open' || d.status === 'under_review');
    const resolvedDisputes = (disputes || []).filter((d: Dispute) => d.status === 'resolved' || d.status === 'dismissed');
        ...
        <div className="space-y-8">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-gray-500">Manage and resolve conflicts between clients and talent.</p>
                </div>
            </div>
                ...
                {openDisputes.length === 0 ? (
                    ...
                    openDisputes.map(dispute => (
                        <MediationCard
                            key={dispute.id}
                            dispute={dispute}
                            onResolve={(res, status) => resolveMutation.mutate({ id: dispute.id, resolution: res, status })}
            ...
            <div className="space-y-6 pt-8 border-t border-gray-100">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                    {resolvedDisputes.map(dispute => (
                        <Card key={dispute.id} className="opacity-80 grayscale-[0.5] hover:opacity-100 hover:grayscale-0 transition-all">
                            <CardContent className="pt-6">
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-2">
                                        <Badge variant="outline" className="text-[10px] uppercase font-bold">Case #{dispute.id.slice(0, 8)}</Badge>
...
function MediationCard({ dispute, onResolve, isResolving }: { dispute: Dispute, onResolve: (res: string, status: 'resolved' | 'dismissed') => void, isResolving: boolean }) {
                ...
                <div className="flex flex-col md:flex-row">
                    <div className="p-8 flex-1">
                        <div className="flex flex-wrap items-center gap-3 mb-6">
                            <Badge className="bg-red-500 text-white font-bold px-3 py-1">#{dispute.id.slice(0, 8)}</Badge>
                            <Badge className="bg-red-50 text-red-600 border-red-100 uppercase tracking-widest text-[10px] font-bold">Immediate Attention</Badge>
                            <span className="text-sm text-gray-400 ml-auto flex items-center gap-1">
                                <ArrowRight className="w-3 h-3" /> Filed {new Date(dispute.createdAt).toLocaleDateString()}
                            </span>
                        </div>
                        ...
                        <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100 mb-6">
                            <h4 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Claim Description</h4>
                            <p className="text-gray-700 leading-relaxed italic">"{dispute.description}"</p>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-gray-400 uppercase">Initiator</p>
                                    <p className="text-sm font-semibold">{dispute.initiator?.full_name}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <AnimatePresence mode="wait">
                            {!isActionOpen ? (
                                ...
                                >
                                    ...
                                    <p className="text-sm text-gray-500 leading-relaxed">As an administrator, your decision is final. Review the case and the legal contracts before resolving.</p>
                                    <Button
                                ...
                                >
                                    ...
                                    <Button variant="ghost" className="w-full text-xs" onClick={() => setIsActionOpen(false)}>Cancel</Button>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>
