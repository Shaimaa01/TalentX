                ...
                <div className="grid lg:grid-cols-3 gap-8">
                    {hiringOptions.map((option, index) => (
                        <motion.div
                            ...
                            className={`relative group bg-white p-8 sm:p-10 rounded-lg border transition-all duration-500 shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] hover:translate-y-[-4px] ${option.featured
                            ...
                            <ul className="space-y-4 mb-10">
                                {option.features.map((feature: string, fIdx: number) => (
                                    <li key={fIdx} className="flex items-center text-sm font-medium text-gray-700">
                                        <CheckCircle2 className="w-5 h-5 mr-3 flex-shrink-0 text-[#00c853]" />
                                        {feature}
                                    </li>
                                ))}
                            </ul>
                            ...
                            <Link href={option.link} className="block mt-auto">
                                <Button
                                    className={`w-full h-10 rounded-full text-base font-bold cursor-pointer transition-all duration-300 ${option.featured
                                        ? 'bg-[#1a1a2e] text-white hover:bg-[#00c853] hover:text-white'
                                        : 'bg-[#1a1a2e] text-white hover:bg-[#00c853] hover:text-white'
                                        }`}
                                >
                                    {option.buttonLabel}
